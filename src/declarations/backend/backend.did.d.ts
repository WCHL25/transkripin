...
export type FileArtifactVisibility = { Private: null } | { Public: null };
export interface UserFileArtifact {
  artifact: FileArtifact;
  is_bookmarked: boolean;
}
export type FileTypeFilter = { Audio: null } | { Video: null };
export type JobStatus =
  | { Failed: string }
  | { Completed: string }
  | { Pending: null };
export type LanguageFilter = { English: null } | { Indonesia: null };
export type Result = { Ok: string } | { Err: string };
export type Result_1 = { Ok: null } | { Err: string };
export type Result_2 = { Ok: FileArtifact } | { Err: string };
export type Result_3 = { Ok: UploadedFile } | { Err: string };
export type Result_4 = { Ok: DownloadChunkResponse } | { Err: string };
export type Result_5 = { Ok: JobStatus } | { Err: string };
export type Result_6 = { Ok: [bigint, bigint] } | { Err: string };
export type SortOrderFilter =
  | { Oldest: null }
